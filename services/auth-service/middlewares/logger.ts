...
class LoggerMiddleware {
  ...
  apiLogger = (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on("finish", () => {
      this.logger.info(
        `${req.method} ${req.originalUrl} - Status: ${res.statusCode} ${res.locals.code != 200 ? `❗Message: ${res?.locals?.message || ""} - Code: ${res?.locals?.code || "" }` : ""} ${res?.locals?.error ? `Error : ${res?.locals?.error}` : ''  }`
      );
    });
  ...
  tempApiLogs = async (payload: TempApiLog) => {
    // try {
    //   await loggerRepository.tempLogs(payload);
    // } catch (error) {
    //   console.log(
    //     "\nProblem with Temp Logger Service ❌❌❌\n" +
    //       error +
    //       "\n Log not inserted into database❌❌❌\n"
    //   );
    // }
  };
  ...
  tempApiLogger = () => {
    return (req: Request, res: Response, next: NextFunction): void => {
      ...
      res.json = (data: any) => {
        ...
        loggerMiddleware.tempApiLogs(tempLog);

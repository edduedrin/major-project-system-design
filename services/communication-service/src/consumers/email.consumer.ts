...
export class EmailNotificationConsumer {
  ...
  public async start(): Promise<void> {
    try {
      ...
      const queueName = config.rabbit.queues.email;

      await this.channel.prefetch(10);
      logger.info(`🎧 Email Notification Consumer listening on queue: ${queueName}`);
      ...
      logger.error("Failed to start Email Notification Consumer", { error: error.message });
    }
  }
  ...
  private async handleMessage(msg: ConsumeMessage): Promise<void> {
    ...
    let event: CommunicationEventPayload<EmailPayload> | null = null;

    try {
      const contentStr = msg.content.toString();
      event = JSON.parse(contentStr) as CommunicationEventPayload<EmailPayload>;
      ...
      const logEntry = await this.queueLogRepo.create({
        ...
        queueName: config.rabbit.queues.email,
      ...
      await this.emailService.processEmailEvent(event);
      ...
      this.channel?.ack(msg);
      logger.info(`Successfully processed EMAIL event for eventId: ${event.eventId}`);
    } catch (error: any) {
      const endTime = Date.now();
      logger.error(`Error processing EMAIL notification for eventId ${event?.eventId}: ${error.message}`);
      ...
      if (event) {
        ...
        const retryEvent = {
          ...
          targetQueue: config.rabbit.queues.email,
